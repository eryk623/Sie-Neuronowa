...
namespace NeuronWeb
{
    ...
    class NeuralNetwork
    {
        ...
        {
            PropagationForward(inputs);
            return neurons.Where(n => n.NeuronNum.Layer == layers.Length - 1).OrderBy(n => n.NeuronNum.Row).Select(n => n.Output).ToArray();
        }
    }
    ...
    class Program
    {
        static void Main()
        {
            var zad1 = new List<(double[], double[])>
            {
            ...
            var zad2 = new List<(double[], double[])>
            {
            ...
            var zad3 = new List<(double[], double[])>
            {
            ...
            {
                case 1:
                    data = zad1; inputCount = 2; outputCount = 1; break;
                case 2:
                    data = zad2; inputCount = 2; outputCount = 2; break;
                case 3:
                    data = zad3; inputCount = 3; outputCount = 2; break;
                default:
                    Console.WriteLine("Nieprawidłowy wybór");return;
            }

            Console.Write("Podaj liczbe warstw ukrytych: ");
            int hl = int.Parse(Console.ReadLine());
            int[] hidden = new int[hl];
            for (int i = 0; i < hl; i++)
            {
                Console.Write($"Neuronów w warstwie ukrytej {i + 1}: ");
                hidden[i] = int.Parse(Console.ReadLine());
            }
            
            var layers = new[] { inputCount }.Concat(hidden).Concat(new[] { outputCount }).ToArray();
            var nn = new NeuralNetwork(layers, learningRate: 0.1);
            nn.Train(data, 50000);

            Console.WriteLine("Wyniki po treningu:");
            foreach (var (inp, tgt) in data)
            {
                var neuron = nn.Result(inp);
                Console.WriteLine($"wejście: [{string.Join(",", inp)}] wartość oczekiwana: [{string.Join(",", tgt)}] wynik: [{string.Join(", ", neuron.Select(v => v.ToString("F3")))}]");
            }
        }
    }
}
